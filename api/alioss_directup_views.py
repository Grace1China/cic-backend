...
class AliOssCallBack_V2(APIView):
    ...
    def post(self,request,*args,**kwargs):
        ...
        try:
            ...
            mfile = MediaFile.objects.update_or_create(name=osskey,church_prefix=data.get('church',''),series_prefix=data.get('seriesrespath', ''),origin_name=data.get('originname',''), mime_type=data.get('mimeType',''),endpoint=get_ALIOSS_DESTINATIONS(data.get('dest', 'source'))['endpoint'],bucket=get_ALIOSS_DESTINATIONS(data.get('dest', 'source'))['bucket'],video_file_status=MediaFile.STATUS_UPLOADED)
            ...
            # 查看HTTP返回码。
            theLogger.info(' add tagging http response status:%s' % result.status)
